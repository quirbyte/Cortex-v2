...
export default function HelpPage() {
    ...
    return (
        <div className="w-full pt-5 p-4 text-manrope bg-zinc-50/50 dark:bg-zinc-950 min-h-screen overflow-y-auto">
            <header className="flex items-center justify-end w-full border-b border-black/5 dark:border-zinc-800 pb-4">
                <div className="flex flex-col items-end w-full">
                    <h1 className="text-xl tracking-tight font-black text-black dark:text-white">Help</h1>
            ...
            <div className="max-w-4xl mx-auto mt-8 flex flex-col gap-10 select-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {supportChannels.map((channel, i) => (
                        <div
                            key={i}
                            className="p-5 bg-white dark:bg-zinc-950 dark:border-zinc-800 border border-black/5 rounded-2xl shadow-xs hover:shadow-sm transition-all flex flex-col justify-between items-start gap-4 group"
                        >
                            <div className="flex flex-col gap-2">
                                <div className="p-2.5 bg-zinc-50 rounded-xl border border-black/3 w-fit">
                                    {channel.icon}
                                </div>
                                <h3 className="text-sm font-bold text-black dark:text-white mt-1">{channel.title}</h3>
                                <p className="text-xs text-black/50 dark:text-white/50 leading-relaxed">{channel.desc}</p>
                            </div>
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2 border-b border-black/5 dark:border-zinc-800 pb-2">
                        <HelpCircle size={15} className="text-black/40 dark:text-white/40" />
                        <h2 className="text-xs font-black uppercase tracking-widest text-black/40 dark:text-white/40">Frequently Asked Questions</h2>
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                        {faqs.map((faq, idx) => (
                            <div
                                key={idx}
                                className="p-5 bg-white border border-black/5 dark:bg-zinc-950 dark:border-zinc-800 rounded-2xl flex flex-col gap-1.5"
                            >
                                <h4 className="text-sm font-bold text-black dark:text-white flex items-center gap-2">
                                    <span className="h-1.5 w-1.5 rounded-full bg-amber-400 shrink-0" />
                                    {faq.question}
                                </h4>
                                <p className="text-xs text-black/50 dark:text-white/50 leading-relaxed pl-3.5">
                                    {faq.answer}
                                </p>
                            </div>
                        ))}
                ...
                <footer className="border-t border-black/5 dark:border-zinc-800 pt-6 text-center">
